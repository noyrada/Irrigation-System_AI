...
from Weather.weather1 import rain_status
...
PROPERTY_ID_RELAY = "4a3fa865-2b96-46c3-81df-4e896ddde5ad"
...
def train_weather_model(dataset_path):
    ...
    dataset.fillna({'Prediction': precip_type_mode}, inplace=True)
...
def control_arduino_relay(properties, thing_id, property_id_control, property_id_relay, action):
...
def main(dataset_path, api_key, city_name):
    ...
    # Example list of Unix timestamps
    timestamps = [1622559200, 1622645600, 1622732000]

    # Convert Unix timestamps to readable format
    readable_timestamps = [datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S') for ts in timestamps]

    # Print the readable timestamps
    for readable_timestamp in readable_timestamps:
    ...
    properties = iot.PropertiesV2Api(iot.ApiClient(client_config))
    ...
    while True:
        ...
        for timestamp, status in zip(readable_timestamps, rain_status):
            if current_hour == datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S').hour and status == 'Rain':
                weather_update = f"Weather Update: {timestamp}: {status}"
                send_telegram_message(weather_update)

            if (current_hour in range(12) and status == "Rain") or (current_hour in range(12, 24) and status == "Rain"):
                control_arduino_relay(properties, THING_ID, PROPERTY_ID_CONTROL, PROPERTY_ID_RELAY, "OFF")
            elif current_hour not in range(12) and moisture_level < 100:
                control_arduino_relay(properties, THING_ID, PROPERTY_ID_CONTROL, PROPERTY_ID_RELAY, "ON")
            elif current_hour in range(frm_datetime1_hour, 12) and moisture_level < 100:
                control_arduino_relay(properties, THING_ID, PROPERTY_ID_CONTROL, PROPERTY_ID_RELAY, "ON")
            elif current_hour in range(frm_datetime2_hour, 24) and moisture_level < 100:
                control_arduino_relay(properties, THING_ID, PROPERTY_ID_CONTROL, PROPERTY_ID_RELAY, "ON")
            else:
                control_arduino_relay(properties, THING_ID, PROPERTY_ID_CONTROL, PROPERTY_ID_RELAY, "OFF")
