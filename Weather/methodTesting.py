...
# Method for train weather model
def train_weather_model(dataset_path):
    # Load dataset
    dataset = pd.read_csv(dataset_path)
    ...
    precip_type_mode = dataset['Prediction'].mode()[0]
    dataset['Prediction'] = dataset['Prediction'].fillna(precip_type_mode)
...
# Method_for_Predict_rain status
...
def main(dataset_path, api_key, city_name):
    ...
    while True:
        ...
        for timestamp, status in zip(readable_timestamps, rain_status):
            ...
            if current_hour == timestamp and status == 'Rain':
                weather_update = f"Weather Update: {timestamp}: {status}"
                send_telegram_message(weather_update)
        if (current_hour in range(12) and rain_status[0] == "Rain") or (
                current_hour in range(12, 24) and rain_status[0] == "Rain"):
